...
namespace ThinkNet.Infrastructure.Core
{
    ...
    /// </summary>
    public class ExcelLibraryHandler : IImportHandler
    {
        #region 属    性
        /// <summary>
        /// 内存数据
        ...
        public ExcelLibraryHandler(String schemaPath, string unquieKey)
        {
            UnquieKey = unquieKey;
        ...
        /// </summary>
        /// <param name="schemaPath"></param>
        public ExcelLibraryHandler(String schemaPath)
        ...
        public virtual List<RowInfo> SourceToTempTable(String filePath)
        {
            ...
            List<RowInfo> dataTable = new List<RowInfo>();
            RowInfo row = null;

            for (int i = DataStartRow; i <= worksheet.Cells.LastRowIndex; i++)
            {
                row = new RowInfo();
                ...
                for (int j = 0; j <= worksheet.Cells.LastColIndex; j++)
                {
                    ...
                    columnInfo.OriginalValue = worksheet.Cells[i, j].Value != null ? worksheet.Cells[i, j].Value.ToString() : string.Empty;
            ...
            foreach (var row in collection)
            {
                foreach (var item in row.Columns)
                {
                    if (item.ColumnName == columnName && item.OriginalValue == oldDataValue + "")
                    {
                        ...
                        {
                            if (item.ColumnName == otherColumnName && item1.OriginalValue == oldOtherDataValue + "")
                            {
                                item.OriginalValue = otherDataValue != null ? otherDataValue.ToString() : string.Empty;
                                item.DataValue = item.OriginalValue;
                                ValidData(item);

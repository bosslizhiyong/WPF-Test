...
namespace ThinkNet.Utility
{
    ...
    /// 数据访问基础类(基于Oracle)
    /// </summary>
    public class OracleHelper : DatabaseHelper
    {
        private OracleConnection _mConnection = null;//数据库连接
        private OracleTransaction _mTransaction = null;//事务处理类
        ...
        public OracleHelper(string strConn)
	    {
            _strConn = strConn;
            _mConnection = new OracleConnection(strConn);
        ...
        public override void Open()
        {
            if (_mConnection.State != ConnectionState.Open)
            {
                ...
                {
                    _mConnection = new OracleConnection(_strConn);
                }
        ...
        public override int ExecuteNonQuery(string strSql, CommandType cmdType, params object[] paras)
        {
            ...
            {
                int result = 0;
                using (OracleCommand cmd = new OracleCommand() { Connection = _mConnection, CommandText = strSql, CommandType = cmdType })
                {
                    if (paras != null)
                    {
        ...
        public override DataTable ExecuteTable(string strSql, CommandType cmdType, params object[] paras)
        {
            try
            {
                ...
                using (OracleCommand cmd = new OracleCommand())
                {
                    ...
                    if (paras != null)
                    {
                        OracleParameter[] cmdParms = (OracleParameter[])paras;

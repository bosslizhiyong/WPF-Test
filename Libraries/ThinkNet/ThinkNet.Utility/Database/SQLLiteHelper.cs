...
namespace ThinkNet.Utility
{
    ...
    /// 数据访问基础类(基于SQLite)
    /// </summary>
    public class SQLLiteHelper : DatabaseHelper
    {
        private SQLiteConnection _mConnection = null;//数据库连接
        private SQLiteTransaction _mTransaction = null;//事务处理类
	    ...
	    {
            _strConn = strConn;
            _mConnection = new SQLiteConnection(strConn);
        ...
        public override void Open()
        {
            if (_mConnection.State != ConnectionState.Open)
            {
                ...
                {
                    _mConnection = new SQLiteConnection(_strConn);
                }
        ...
        public override int ExecuteNonQuery(string strSql, CommandType cmdType, params object[] paras)
        {
            ...
            {
                int result = 0;
                using (SQLiteCommand cmd = new SQLiteCommand() { Connection = _mConnection, CommandText = strSql, CommandType = cmdType })
                {
                    if (paras != null)
                    {
                        foreach (SQLiteParameter item in paras)
                        {
        ...
        public override DataTable ExecuteTable(string strSql, CommandType cmdType, params object[] paras)
        {
            try
            {
                ...
                using (SQLiteCommand cmd = new SQLiteCommand())
                {
                    ...
                    if (paras != null)
                    {
                        SQLiteParameter[] cmdParms = (SQLiteParameter[])paras;

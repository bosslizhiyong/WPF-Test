...
namespace ThinkNet.Utility
{
    ...
    public class MySQLHelper : DatabaseHelper
    {
        ...
        private MySqlTransaction _mTransaction = null;//事务处理类
	    ...
	    {
            _strConn = strConn;
            _mConnection = new MySqlConnection(strConn);
        ...
        public override void Open()
        {
            if (_mConnection.State != ConnectionState.Open)
            {
                ...
                {
                    _mConnection = new MySqlConnection(_strConn);
                }
        ...
        public override int ExecuteNonQuery(string strSql, CommandType cmdType, params object[] paras)
        {
            ...
            {
                int result = 0;
                using (MySqlCommand cmd = new MySqlCommand() { Connection = _mConnection, CommandText = strSql, CommandType = cmdType })
                {
                    if (paras != null)
                    {
        ...
        public override DataTable ExecuteTable(string strSql, CommandType cmdType, params object[] paras)
        {
            try
            {
                ...
                using (MySqlCommand cmd = new MySqlCommand())
                {
                    ...
                    if (paras != null)
                    {
                        MySqlParameter[] cmdParms = (MySqlParameter[])paras;

...
namespace ThinkNet.Utility
{
    ...
    /// 数据访问基础类(基于OleDB)
    /// </summary>
    public class OleDBHelper : DatabaseHelper
    {
        private OleDbConnection _mConnection = null;//数据库连接
        private OleDbTransaction _mTransaction = null;//事务处理类
	    ...
	    {
            _strConn = strConn;
            _mConnection = new OleDbConnection(strConn);
        ...
        public override void Open()
        {
            if (_mConnection.State != ConnectionState.Open)
            {
                ...
                {
                    _mConnection = new OleDbConnection(_strConn);
                }
        ...
        public override int ExecuteNonQuery(string strSql, CommandType cmdType, params object[] paras)
        {
            ...
            {
                int result = 0;
                using (OleDbCommand cmd = new OleDbCommand() { Connection = _mConnection, CommandText = strSql, CommandType = cmdType })
                {
                    if (paras != null)
                    {
        ...
        public override DataTable ExecuteTable(string strSql, CommandType cmdType, params object[] paras)
        {
            try
            {
                ...
                using (OleDbCommand cmd = new OleDbCommand())
                {
                    ...
                    if (paras != null)
                    {
                        OleDbParameter[] cmdParms = (OleDbParameter[])paras;
